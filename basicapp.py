...
order_product = db.Table('order-Table',db.Column('order_id',db.Integer,db.ForeignKey('order.id'),primary_key = True),db.Column('product_id',db.Integer,db.ForeignKey('product.id'),primary_key = True))
...
def add_orders():
    ...
    for _ in range(1000):
        ...
        coupon_code = random.choices([None,'50OFF','FREESHIPPIING','BUYONEGETONE'],[80,5,5,5])[0]
        order = Order(
...
def orders_with_code():
    ...
    orders = Order.query.filter(Order.coupon_code.isnot(None)).filter(Order.coupon_code != 'FREESHIPPIING').all()
    for order in orders:
        print(order.coupon_code)

def revenue_in_last_x_days(x_days=30):
    print('Revenue in past {} days'.format(x_days))
    print(db.session.query(db.func.sum(Product.price)).join(order_product).join(Order).filter(Order.order_date > (datetime.now() - timedelta(days=x_days))).scalar())

def average_fulfillment_time():
    print('Average fulfillment time')
    print(db.session.query(db.func.time(db.func.avg(db.func.strftime('%s',Order.shipped_date)-db.func.strftime('%s',Order.order_date)),'unixepoch')).filter(Order.shipped_date.isnot(None)).scalar())

def get_customers_purchased_greater_than_x_dollars(amount=500):
    print('All the customers purchased greater than {} dollars'.format(amount))
